...
global mm_cd_model
mm_cd_model = load_model('models/mm_cd.h5')
...
def annotate_img (imgWarp_rs,mm_cd_pred,reading,savename):
...
def score(path_to_image,savename):
	...
	#predict bounding box and point projection for transpose
	model_input_shape = xy_wh_pp_model.input_shape[1]
	dim = (model_input_shape, model_input_shape)
	image = cv2.resize(square_img, dim, interpolation = cv2.INTER_AREA)
	xy_wh_pp_pred = xy_wh_pp_model.predict(image.reshape(1,model_input_shape,
											model_input_shape,3))
	...
	#resize to required for mm_cd_model
	model_input_shape = mm_cd_model.input_shape[1]
	dim = (model_input_shape, model_input_shape)
	imgWarp_rs = cv2.resize(warped_image, dim, interpolation = cv2.INTER_AREA)

	#make kpts prediction
	mm_cd_pred = mm_cd_model.predict(imgWarp_rs.reshape(1,model_input_shape,model_input_shape,3))
	...
	#annotate the image
	annotate_img(imgWarp_rs,mm_cd_pred,reading,savename)
...
if __name__ == "__main__":
	...
	#start timer to measure infrence rate
	...
	infrence_rate = (endtime - starttime)/len(files)
	print('infrence rate = ' + str(round(infrence_rate,2)) + 's per image')
